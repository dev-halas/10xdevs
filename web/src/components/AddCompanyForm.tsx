...
export default function AddCompanyForm({ onSuccess, onCancel }: AddCompanyFormProps) {
  ...
  const handleChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    // Wyczyść błąd przy zmianie wartości
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Normalizuj dane - usuń spacje i myślniki
      ...
      await companiesService.createCompany(normalizedData);
      
      setSuccess(true);
      setFormData({ name: '', nip: '', regon: '' });
      ...
      setError(err instanceof Error ? err.message : 'Błąd podczas dodawania firmy');
    } finally {
      setLoading(false);
    }
  };
    ...
    return (
      <div className={styles.container}>
        <div className={styles.card}>
          ...
          <p>Firma została pomyślnie dodana.</p>
          <button 
    ...
    <div className={styles.container}>
      <div className={styles.card} style={{ maxWidth: '500px' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px' }}>
          <h2 style={{ margin: 0 }}>Dodaj nową firmę</h2>
          {onCancel && (
            <button 
              onClick={handleCancel}
              className={styles.btn}
        ...
        <div style={{ 
          ...
          <ul style={{ margin: '8px 0', paddingLeft: '20px' }}>
            ...
            <li>Możesz używać myślników w NIP i REGON - zostaną automatycznie usunięte</li>
          </ul>
        </div>
      </div>
    </div>
